...
// Track cycle counts
struct perf_results abac_rules_perf_store;
...
static int resolve(avp *user_attr, obj_rule *head, enum operation op, uint64_t *cycle_counts){
	 ...
	 * 3. Current environmental attributes (avp *abac_rules_env_attr -> from abacfs)
...
static int abac_file_permission(struct file *file, int mask)
{
	...
	*/
	...
	save_tsc_stop(cycle_counts, uid, &abac_rules_perf_store);
...
static int __init abac_init(void)
{
	...
	perf_results_init(&abac_rules_perf_store);
