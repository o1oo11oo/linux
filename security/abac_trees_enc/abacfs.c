...
struct dentry *abac_trees_enc_generic_abacfs;
struct dentry *abac_trees_enc_generic_user_attr_file;
struct dentry *abac_trees_enc_generic_obj_attr_file;
struct dentry *abac_trees_enc_generic_env_attr_file;
struct dentry *abac_trees_enc_generic_action_file;
struct dentry *abac_trees_enc_generic_perf_file;
...
{
	if (!IS_ERR_OR_NULL(abac_trees_enc_user_attr_file)) {
		securityfs_remove(abac_trees_enc_user_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_obj_attr_file)) {
		securityfs_remove(abac_trees_enc_obj_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_env_attr_file)) {
		securityfs_remove(abac_trees_enc_env_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_action_file)) {
		securityfs_remove(abac_trees_enc_action_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_perf_file)) {
		securityfs_remove(abac_trees_enc_perf_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_abacfs)) {
		securityfs_remove(abac_trees_enc_abacfs);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_user_attr_file)) {
		securityfs_remove(abac_trees_enc_generic_user_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_obj_attr_file)) {
		securityfs_remove(abac_trees_enc_generic_obj_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_env_attr_file)) {
		securityfs_remove(abac_trees_enc_generic_env_attr_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_action_file)) {
		securityfs_remove(abac_trees_enc_generic_action_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_perf_file)) {
		securityfs_remove(abac_trees_enc_generic_perf_file);
	}
	if (!IS_ERR_OR_NULL(abac_trees_enc_generic_abacfs)) {
		securityfs_remove(abac_trees_enc_generic_abacfs);
	}
}
...
static struct dentry *create_file(struct dentry *parent, const char *parentname, const char *filename, const struct file_operations *fops) {
	...
	if (IS_ERR(f)) {
		printk(KERN_ERR "ABAC LSM (Trees ENC): Failed to create file /sys/kernel/security/%s/%s", parentname, filename);
		destroy_abac_fs();
		return f;
	}
	printk(KERN_INFO "ABAC LSM (Trees ENC): Created file /sys/kernel/security/%s/%s", parentname, filename);
	return f;
}
...
{
	const char *parentname = "abac_trees_enc";

	if (!abac_trees_enc_initialized) {
		pr_info("ABAC LSM (Trees ENC): LSM was not initialized, not loading securityfs");
		return 0;
	}

	// create the root 'abac' directory
	abac_trees_enc_abacfs = securityfs_create_dir(parentname, NULL);
	if (IS_ERR(abac_trees_enc_abacfs)) {
		printk(KERN_ERR "ABAC LSM (Trees ENC): Failed to create abac securityfs at /sys/kernel/security/%s/", parentname);
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_abacfs);
	}

	abac_trees_enc_user_attr_file = create_file(abac_trees_enc_abacfs, parentname, "user_attr", &user_attr_fops);
	if (IS_ERR(abac_trees_enc_user_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_user_attr_file);
	}
	abac_trees_enc_obj_attr_file = create_file(abac_trees_enc_abacfs, parentname, "obj_attr", &obj_attr_fops);
	if (IS_ERR(abac_trees_enc_obj_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_obj_attr_file);
	}
	abac_trees_enc_env_attr_file = create_file(abac_trees_enc_abacfs, parentname, "env_attr", &env_attr_fops);
	if (IS_ERR(abac_trees_enc_env_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_env_attr_file);
	}

	// Performance evaluation files
	abac_trees_enc_action_file = create_file(abac_trees_enc_abacfs, parentname, "action", &action_fops);
	if (IS_ERR(abac_trees_enc_action_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_action_file);
	}
	abac_trees_enc_perf_file = create_file(abac_trees_enc_abacfs, parentname, "perf", &perf_fops);
	if (IS_ERR(abac_trees_enc_perf_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_perf_file);
	}

	printk(KERN_INFO "ABAC LSM (Trees ENC): Securityfs Initialized");
	return 0;
}
...
static int abac_create_generic_fs(void)
{
	...
	if (!abac_trees_enc_initialized) {
		pr_info("ABAC LSM (Trees ENC): LSM was not initialized, not loading generic securityfs");
		return 0;
	}

	if (abac_rules_initialized || abac_rules_enc_initialized || abac_trees_initialized) {
		pr_info("ABAC LSM (Trees ENC): We are not the only ABAC LSM, not using generic paths");
		return 0;
	}

	pr_info("ABAC LSM (Trees ENC): We are the only ABAC LSM, using generic paths additionally");

	// create the root 'abac' directory
	abac_trees_enc_generic_abacfs = securityfs_create_dir(parentname, NULL);
	if (IS_ERR(abac_trees_enc_generic_abacfs)) {
		printk(KERN_ERR "ABAC LSM (Trees ENC): Failed to create abac securityfs at /sys/kernel/security/%s/", parentname);
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_abacfs);
	}

	abac_trees_enc_generic_user_attr_file = create_file(abac_trees_enc_generic_abacfs, parentname, "user_attr", &user_attr_fops);
	if (IS_ERR(abac_trees_enc_generic_user_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_user_attr_file);
	}
	abac_trees_enc_generic_obj_attr_file = create_file(abac_trees_enc_generic_abacfs, parentname, "obj_attr", &obj_attr_fops);
	if (IS_ERR(abac_trees_enc_generic_obj_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_obj_attr_file);
	}
	abac_trees_enc_generic_env_attr_file = create_file(abac_trees_enc_generic_abacfs, parentname, "env_attr", &env_attr_fops);
	if (IS_ERR(abac_trees_enc_generic_env_attr_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_env_attr_file);
	}

	// Performance evaluation files
	abac_trees_enc_generic_action_file = create_file(abac_trees_enc_generic_abacfs, parentname, "action", &action_fops);
	if (IS_ERR(abac_trees_enc_generic_action_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_action_file);
	}
	abac_trees_enc_generic_perf_file = create_file(abac_trees_enc_generic_abacfs, parentname, "perf", &perf_fops);
	if (IS_ERR(abac_trees_enc_generic_perf_file)) {
		destroy_abac_fs();
		return PTR_ERR(abac_trees_enc_generic_perf_file);
	}

	printk(KERN_INFO "ABAC LSM (Trees ENC): Generic Securityfs Initialized");
	return 0;
}
