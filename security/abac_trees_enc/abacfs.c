...
extern struct perf_results abac_trees_enc_perf_store;
...
const size_t ABAC_TREES_ENC_PERF_FILE_SIZE = 31457280; // 30MB
...
static ssize_t perf_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	perf_results_stop_recording(&abac_trees_enc_perf_store);

	// Allocate buffer
	kbuf = kvmalloc(ABAC_TREES_ENC_PERF_FILE_SIZE, GFP_KERNEL);
	if (!kbuf)
		return -ENOMEM;

	// Fill with JSON output
	perf_results_serialize_to_json(&abac_trees_enc_perf_store, kbuf, ABAC_TREES_ENC_PERF_FILE_SIZE);
...
static ssize_t perf_write(struct file *file, const char __user *ubuf, size_t len, loff_t *ppos)
{
	...
	if (strcmp(kbuf, "start") == 0) {
		...
		perf_results_start_recording(&abac_trees_enc_perf_store);
	} else if (strcmp(kbuf, "clear") == 0) {
		pr_info("Clearing perf data\n");
		perf_results_clear_entries(&abac_trees_enc_perf_store);
	} else if (strcmp(kbuf, "reset") == 0) {
		pr_info("Resetting perf data storage\n");
		perf_results_free(&abac_trees_enc_perf_store);
	} else {
		ret = kstrtoint(kbuf, 10, &parsed);
		if (ret)
			return ret;
		pr_info("Registering perf runner %d with %d entries\n", uid, parsed);
		perf_results_register_runner(&abac_trees_enc_perf_store, uid, parsed);
	}
