...
pub(crate) fn resolve(
    ...
    for (idx, rule) in rules.iter().enumerate() {
        // Calculate cache key and reset the hasher for the next iteration
        hash_state.update(&operation.to_ne_bytes())?;
        hash_state.update(&idx.to_ne_bytes())?;
        hash_state.update(user_attr.as_bytes())?;
        hash_state.update(object_attr.as_bytes())?;
