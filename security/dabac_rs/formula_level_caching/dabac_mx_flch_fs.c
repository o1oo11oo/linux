...
{
	if (dabac_rs_mx_flch_user_attr_file)
		securityfs_remove(dabac_rs_mx_flch_user_attr_file);
	if (dabac_rs_mx_flch_obj_attr_file)
		securityfs_remove(dabac_rs_mx_flch_obj_attr_file);
	if (dabac_rs_mx_flch_env_attr_file)
		securityfs_remove(dabac_rs_mx_flch_env_attr_file);
	if (dabac_rs_mx_flch_policy_file)
		securityfs_remove(dabac_rs_mx_flch_policy_file);
	if (dabac_rs_mx_flch_fs)
		securityfs_remove(dabac_rs_mx_flch_fs);
}

static struct dentry *dabac_rs_mx_flch_create_file(const char *filename, const struct file_operations *fops)
{
	struct dentry *f;

	// File is writable by everyone because the PAP checks each access with the PDP
	f = securityfs_create_file(filename, 0666, dabac_rs_mx_flch_fs, NULL, fops);
	if (!f) {
...
static int dabac_rs_mx_flch_create_fs(void)
{
	...
	dabac_rs_mx_flch_fs = securityfs_create_dir("dabac_rs", NULL);
	if (!dabac_rs_mx_flch_fs) {
	...
	dabac_rs_mx_flch_user_attr_file = dabac_rs_mx_flch_create_file("user_attr", &user_attr_fops);
	if (!dabac_rs_mx_flch_user_attr_file) {
		dabac_rs_mx_flch_destroy_fs();
		return -1;
	}
	dabac_rs_mx_flch_obj_attr_file = dabac_rs_mx_flch_create_file("obj_attr", &obj_attr_fops);
	if (!dabac_rs_mx_flch_obj_attr_file) {
		dabac_rs_mx_flch_destroy_fs();
		return -1;
	}
	dabac_rs_mx_flch_env_attr_file = dabac_rs_mx_flch_create_file("env_attr", &env_attr_fops);
	if (!dabac_rs_mx_flch_env_attr_file) {
		dabac_rs_mx_flch_destroy_fs();
		return -1;
	}
	dabac_rs_mx_flch_policy_file = dabac_rs_mx_flch_create_file("policy", &policy_fops);
	if (!dabac_rs_mx_flch_policy_file) {
		dabac_rs_mx_flch_destroy_fs();
		return -1;
	}
