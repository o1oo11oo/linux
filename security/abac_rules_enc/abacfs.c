...
const size_t ABAC_RULES_ENC_PERF_FILE_SIZE = 31457280; // 30MB
...
static ssize_t perf_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	// Allocate buffer
	kbuf = kvmalloc(ABAC_RULES_ENC_PERF_FILE_SIZE, GFP_KERNEL);
	if (!kbuf)
		return -ENOMEM;

	// Fill with JSON output
	perf_results_serialize_to_json(&abac_rules_enc_perf_store, kbuf, ABAC_RULES_ENC_PERF_FILE_SIZE);
