...
DECLARE_HASHTABLE(abac_rules_enc_obj_rule_map, OBJ_BUCKETS);
...
void abac_rules_enc_parse_obj_rule_map(char *data) {
	struct abac_obj *temp;
	struct obj_hnode *o;
	char *line;

	hash_init(abac_rules_enc_obj_rule_map);
	...
	while((line = strsep(&data, "\n")) != NULL) {
		...
		o->head = temp->head;
		hash_add(abac_rules_enc_obj_rule_map, &(o->node), simple_hash(o->path));
		printk("Added %s to hashtable", o->path);
	}
}

obj_rule *abac_rules_enc_get_obj_rule_list(char *path) {
	/* Get rules mapped to object at a given path */
	struct obj_hnode *cur;
	obj_rule *head; 
	u32 key = simple_hash(path);
	head = NULL;
	hash_for_each_possible(abac_rules_enc_obj_rule_map, cur, node, key) {
...
void abac_rules_enc_clear_obj_rule_map(void) {
    ...
    hash_for_each(abac_rules_enc_obj_rule_map, bkt, cur, node) {
		clear_rule_list(cur->head);
		hash_del(&(cur->node));
    }
}

void abac_rules_enc_print_obj_rule_list(obj_rule *r) {
	while (r != NULL) {
		printk("%u-", r->id);
		r = r->next;
	}
...
void abac_rules_enc_print_obj_rule_map(void) {
    ...
    hash_for_each(abac_rules_enc_obj_rule_map, bkt, cur, node) {
		...
		abac_rules_enc_print_obj_rule_list(cur->head);
    }
}
