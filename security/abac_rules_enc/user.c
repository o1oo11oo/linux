...
DECLARE_HASHTABLE(abac_rules_enc_user_attr_map, USER_BUCKETS);
...
static struct abac_user *parse_line(char *line) {
	...
	usr->attrs = abac_rules_enc_parse_avp(line);
	return usr;
}

void abac_rules_enc_parse_user_attr(char *data) {
	/*
	...
	hash_init(abac_rules_enc_user_attr_map);
	...
	while((line = strsep(&data, "\n")) != NULL) {
		...
		u->uid = temp->uid;
		u->attrs = temp->attrs;
		hash_add(abac_rules_enc_user_attr_map, &(u->node), u->uid);
		printk("Added %u to hashtable", u->uid);
	}
}

avp *abac_rules_enc_get_user_attrs(unsigned int uid) {
	/* Get user attributes mapped to a UID */
	struct user_hnode *cur;
	avp *attrs = NULL;
	hash_for_each_possible(abac_rules_enc_user_attr_map, cur, node, uid) {
		attrs = cur->attrs;
		break;
	}
	return attrs;
...
void abac_rules_enc_clear_user_attrs(void) {
    ...
    hash_for_each(abac_rules_enc_user_attr_map, bkt, cur, node) {
		abac_rules_enc_clear_avp_list(cur->attrs);
		hash_del(&(cur->node));
    }
}
...
void abac_rules_enc_print_user_attrs(void) {
    ...
    hash_for_each(abac_rules_enc_user_attr_map, bkt, cur, node) {
		...
		abac_rules_enc_print_avp(cur->attrs);
    }
}
